...
/*	
	...
	private void addAllQueries(){
		...
		query = 
		" SELECT i.imagereference_uid, COUNT(*) AS count " +
		" FROM  pais.markup_polygon m, " +
		" PAIS.WHOLESLIDEIMAGEREFERENCE i, PAIS.COLLECTION c " +
		" WHERE i.PAIS_UID = c.PAIS_UID AND " +
		" m.pais_uid=c.pais_uid AND " +
		...
		query = "SELECT pais_uid, COUNT(*) AS count " +
				"FROM   pais.markup_polygon " +
				"GROUP BY pais_uid";
		map.put(name, query);	
...
/*		name = "getBoundariesFromPaisUidFromTileWithParam";
		...
		* Example query: 
		...
		" FROM PI.IMAGE I order by I.imagereference_uid";
		map.put(name, query);	
		...
		query = 
		" SELECT I.IMAGEREFERENCE_UID,I.NAME " +
		...
		query = 
		" SELECT i.name,i.imagereference_uid,p.patientid AS PATIENTID" +
		" FROM PI.IMAGE i,pi.patient p where i.patient_id=p.id order by i.imagereference_uid";
		map.put(name, query);	
...
/*		name = "getRegionImageDummy";
		...
		name = "getImageLocationPathByPAISUID";
		query = 
		" SELECT CONCAT (L.FOLDER, I.NAME) " +
		...
		name="paisuidsFromImageReferenceUID";
		query="select w.pais_uid,c.name,c.role,c.methodname,c.sequencenumber,c.studydatetime from pais.wholeslideimagereference w,pais.collection c where w.imagereference_uid=? and c.pais_uid=w.pais_uid order by w.pais_uid";
		map.put(name, query);
		...
		name="regionsfromPAISUID";
		query = "select x,y,height,width,name from pais.region where pais_uid=?";
		map.put(name, query);
		
		/*human annotation
		 * */
		name = "annotationByID";
		query = "select boundary,description from pais.human_annotation where annotation_id=?";
		map.put(name, query);
		
		name = "annotationsByimageID";
		query = "select boundary,description from pais.human_annotation " +
				"where imagereference_uid=? " +
				"AND DB2GSE.ST_Contains(DB2GSE.ST_Polygon( CAST (? AS VARCHAR(30000) ), 1), boundary_box) = 1";
		map.put(name, query);
		
		name = "insertHumanAnnotation";
		query = "insert into pais.human_annotation(imagereference_uid,boundary,description,boundary_box,insert_time) values(?,?,?,DB2GSE.ST_Polygon( CAST (? AS VARCHAR(30000) ), 1),CURRENT_TIMESTAMP)";
		map.put(name, query);
